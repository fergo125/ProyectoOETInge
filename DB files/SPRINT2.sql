...
CREATE OR REPLACE TRIGGER "TRIGGER_ACTUALIZA_SALDO_VENTA" AFTER INSERT ON REGISTRO_DETALLES_FACTURAS
  ...
  BEGIN
      --Se averigua en qu? bodega estamos trabajando
      SELECT  BODEGA
...
--DELETE FROM SEG_USUARIO_BODEGA WHERE ESTACION = '' --Todas las bodegas deberú}n estar asociadas a una estación
...
-- Se agrega la columna MAXIMO_DESCUENTO en INV_PRODUCTOS y SEG_USUARIO, y se inicializan todos los valores de esa columna con valores predeterminados

ALTER TABLE INV_PRODUCTOS ADD DESCUENTO_MAXIMO INT;
UPDATE INV_PRODUCTOS SET DESCUENTO_MAXIMO = 20;
